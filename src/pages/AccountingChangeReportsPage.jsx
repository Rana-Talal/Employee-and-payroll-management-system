...
export default function AccountingChangeReportsPage() {
  ...
  const fetchReports = async () => {
    try {
      ...
      // فلترة التقارير: فقط التي تحتاج موافقة الحسابات
      const accountingReports = items.filter(r =>
        r.requiresAccountingApproval && !r.accountingApproved
      );

      setReports(accountingReports);

      const statsData = {
        total: accountingReports.length,
        pending: accountingReports.filter(r => !r.accountingApproved).length,
        approved: items.filter(r => r.accountingApproved).length,
      };
      setStats(statsData);
    } catch (error) {
  ...
  const handleRejectSubmit = async () => {
    ...
    try {
      const res = await fetch(`${BASE_URL}/ChangeReports/RejectAccounting/${reportToReject.changeReportID}`, {
        method: "POST",
        headers: {
          ...getAuthHeaders(),
      ...
      showToast("تم رفض التقرير وإرسال إشعار للموارد البشرية", "success");
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 p-6" dir="rtl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <div className="bg-white rounded-2xl shadow-lg border-2 border-slate-200 p-5 hover:shadow-xl transition-shadow">
          ...
          <p className="text-sm font-bold text-blue-600">التقارير المعلقة</p>
        </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full mx-4 mb-8" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-8">
              ...
              <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-200">
                ...
                <button
                  onClick={() => handleApproveAccounting(selectedReport.changeReportID)}
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-6">
              <p className="text-gray-700 mb-4">
                سيتم إرسال إشعار لقسم الموارد البشرية مع سبب الرفض
              </p>
              <div className="mb-6">
