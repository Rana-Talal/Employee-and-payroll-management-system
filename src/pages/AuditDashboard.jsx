...
export default function AuditDashboard() {
  ...
  const [stats, setStats] = useState({
    totalEmployees: 0,
    totalReports: 0,
    pendingAuditReports: 0,
    auditApprovedReports: 0,
  });
    ...
    const fetchUnreadCount = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // حساب الرسائل غير المقروءة للتدقيق
          // التدقيق يستقبل الطلبات التي وافقت عليها الحسابات ولم يوافق عليها التدقيق بعد
          const auditMessages = changeReports.filter(report =>
            report.requiresAuditApproval &&
            report.accountingApproved === true &&
            report.auditApproved === null
          );

          setUnreadCount(auditMessages.length);
        } else {
          setUnreadCount(0);
        }
      } catch (error) {
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        setStats({
          ...
          pendingAuditReports: reportsList.filter(r =>
            r.changeStatus === 3 && r.accountingApproved && !r.auditApproved
          ).length,
  ...
  const StatCard = ({ title, value, icon, color, onClick, clickable = false }) => (
    <div
      ...
      className={`bg-white p-6 rounded-lg shadow-sm border border-gray-200 transition-all duration-300 ${
        clickable
          ? 'hover:shadow-xl hover:scale-105 cursor-pointer hover:border-purple-300 active:scale-95'
          : 'hover:shadow-md'
      }`}
    >
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex" dir="rtl">
      ...
      <aside className="w-80 bg-white border-l border-gray-200 shadow-lg flex-shrink-0 sticky top-0 h-screen overflow-y-auto">
        <div className="p-6">
          ...
          <div className="mb-6 pb-4 border-b border-gray-200">
            ...
            <p className="text-sm text-gray-500 mt-1">مراجعة واعتماد التقارير</p>
          </div>
          ...
          <nav className="space-y-2">
            ...
            {/* صفحة القراءة */}
            {/* <button
              onClick={() => navigate("/audit-department")}
              className="w-full text-right px-3 py-2.5 rounded-lg bg-gray-50 hover:bg-gray-100 text-gray-700 text-sm font-medium transition flex items-center gap-2 group"
            >
              <span className="text-lg group-hover:scale-110 transition-transform">📖</span>
              <span>صفحة المراجعة</span>
            </button> */}

            {/* تقارير التغييرات والموافقات */}
            <button
              onClick={() => navigate("/audit-change-reports")}
              className="w-full text-right px-3 py-2.5 rounded-lg bg-gray-50 hover:bg-gray-100 text-gray-700 text-sm font-medium transition flex items-center gap-2 group"
            >
              <span className="text-lg group-hover:scale-110 transition-transform">📊</span>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <StatCard
              ...
              value={stats.pendingAuditReports}
              icon="⏳"
            ...
            <StatCard
              ...
              value={stats.auditApprovedReports}
              icon="✅"
          ...
          {/* Recent Activity */}
          {stats.pendingAuditReports > 0 && (
            <div className="mt-6 bg-gradient-to-r from-purple-50 to-pink-50 border-l-4 border-purple-500 p-6 rounded-lg">
              <div className="flex items-start gap-3">
                <div className="text-3xl">⚠️</div>
                <div>
                  <h3 className="font-bold text-gray-800 mb-1">تنبيه: تقارير تحتاج للمراجعة</h3>
                  <p className="text-gray-700 mb-3">
                    لديك <span className="font-bold text-purple-600">{stats.pendingAuditReports}</span> تقرير تغيير بانتظار موافقة التدقيق
                  </p>
                  <button
                    onClick={() => navigate("/audit-change-reports")}
      ...
      {/* Message Modals */}
      {showInbox && <MessageInbox department="Audit" onClose={() => setShowInbox(false)} />}
      {showOutbox && <MessageOutbox department="Audit" onClose={() => setShowOutbox(false)} />}
    </div>
  );
}
